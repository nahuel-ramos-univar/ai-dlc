...
- **Context:** `/sync-context` writes a short repository index under the consumer repository's `aidlc-docs/` and selectively useful `<module-root>/AIDLC_CONTEXT.md` files. It uses an artifact-home fallback only when source roots cannot be written. It proposes BUGBOT guidance separately and writes it only after a per-repository approval.
